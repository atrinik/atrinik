...
socket_command_map_validate_level(map_packet_reader_t *reader, int wire_width, int wire_height) {
    ...
    while (reader->pos < reader->len) {
        ...
        if ((values & MAP2_MASK_LIGHT_LEVEL_MORE) && !map_packet_skip(reader, NUM_SUB_LAYERS - 1)) {
            return false;
        }

        uint8_t num_layers;
        if (!map_packet_read_uint8(reader, &num_layers) || num_layers > NUM_REAL_LAYERS) {
            return false;
        }

        bool seen_layers[NUM_REAL_LAYERS] = {0};
        for (uint8_t i = 0; i < num_layers; i++) {
            ...
            if (layer >= NUM_REAL_LAYERS || seen_layers[layer]) {
                return false;
            }
        ...
        if (ext_flags & MAP2_FLAG_EXT_ANIM) {
            uint8_t anim_num;
            bool seen_sub_layers[NUM_SUB_LAYERS] = {0};

            if (!map_packet_read_uint8(reader, &anim_num) || anim_num > NUM_SUB_LAYERS) {
                return false;
            }
            ...
            for (uint8_t i = 0; i < anim_num; i++) {
                ...
                if (!map_packet_read_uint8(reader, &sub_layer) ||
                    !map_packet_read_uint8(reader, &anim_type) || sub_layer >= NUM_SUB_LAYERS ||
                    seen_sub_layers[sub_layer] ||
...
static bool socket_command_map_validate(const uint8_t *data, size_t len, size_t pos) {
    map_packet_reader_t reader = {.data = data, .len = len, .pos = pos};
    uint8_t mapstat;
    int map_width_limit = MAP_LOOK_TO_WIRE_SIZE(setting_get_int(OPT_CAT_MAP, OPT_MAP_WIDTH));
    int map_height_limit = MAP_LOOK_TO_WIRE_SIZE(setting_get_int(OPT_CAT_MAP, OPT_MAP_HEIGHT));
    ...
    if (!map_packet_read_uint8(&reader, &xpos) || !map_packet_read_uint8(&reader, &ypos) ||
        !map_packet_read_uint8(&reader, &player_sub_layer) || player_sub_layer >= NUM_SUB_LAYERS ||
...
void socket_command_map(uint8_t *data, size_t len, size_t pos) {
    ...
    if (!socket_command_map_validate(data, len, pos)) {
