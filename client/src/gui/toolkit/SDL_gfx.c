...
/*!
 *    \brief Internal function to move Bresenham line iterator to the next
 * position.
 *
 ...
 *    \param b Pointer to struct for bresenham line drawing state.
 *
 *    \returns Returns 0 on success, 1 if last point was reached, 2 if moving
 * past end-of-line, -1 on failure.
 */
...
/*!
 *    \brief Internal pixel drawing - fast, no blending, no locking, no
 * clipping.
 *
 ...
 * color in in destination format.
 *
 *    Contains two alternative 32 bit alpha blending routines which can be
 * enabled at the source
 *    level with the defines DEFAULT_ALPHA_PIXEL_ROUTINE or
 * EXPERIMENTAL_ALPHA_PIXEL_ROUTINE.
 ...
 *    \param x2 X coordinate of the second corner (lower right) of the
 * rectangle.
 *    \param y2 Y coordinate of the second corner (lower right) of the
 * rectangle.
 ...
 *
 *    Just stores the color value (including the alpha component) without
 * blending.
 ...
 *
 *    \param dst The surface to draw on.
 *    \param x X coordinate of the first point (i.e. top right) of the
 * rectangle.
 *    \param y Y coordinate of the first point (i.e. top right) of the
 * rectangle.
 *    \param x2 X coordinate of the second point (i.e. bottom left) of the
 * rectangle.
 *    \param y2 Y coordinate of the second point (i.e. bottom left) of the
 * rectangle.
 *    \param color The color value of the rectangle to draw (0xRRGGBBAA).
 *
 *    \returns Returns 0 on success, -1 on failure.
 ...
 *
 *    Calculates a segement-based clipping encoding for a point against a
 * rectangle.
 *
 *    \param x X coordinate of point.
 *    \param y Y coordinate of point.
 ...
 *    \param draw_endpoint Flag indicating if the endpoint should be drawn; draw
 * if non-zero.
 *
 *    \returns Returns 0 on success, -1 on failure.
 */
int _aalineColor(SDL_Surface * dst, Sint16 x, Sint16 y, Sint16 x2, Sint16 y2, Uint32 color, int draw_endpoint)
{
    ...
    if (dy > dx) {
         ...
         * that
         * X advances every time Y advances 1 pixel, truncating the result so
         * that
         ...
         * pixel
         * that Y advances each time X advances 1 pixel, truncating the result
         * so
 ...
 *    \param end Ending radius in degrees of the arc. 0 degrees is down,
 * increasing counterclockwise.
...
int arcColor(SDL_Surface * dst, Sint16 x, Sint16 y, Sint16 rad, Sint16 start, Sint16 end, Uint32 color)
{
    ...
    do {
        ...
        if (oct == startoct) {
             ...
             * draw, so we */
            /* stop immediately after we render the last sensible pixel at x =
             * ((int)temp). */

            /* and whether to draw in this octant initially */
            if (oct % 2) drawoct |= (1 << oct);  /* this is basically like
                                                  * saying
                                                  * drawoct[oct] = true, if
                                                  * drawoct were a bool array */
            else drawoct &= 255 - (1 << oct);         /* this is basically like
                                                       * saying drawoct[oct] =
                                                       * false */
        }
        ...
        if (oct == endoct) {
            ...
            if (startoct == endoct) {
                ...
                if (start > end) {
                     ...
                     * draw over the whole circle, */
                    /* we need to set the rest to true, because the while loop
...
/*!
 *    \brief Internal float (low-speed) pie-calc implementation by drawing
 * polygons.
 *
 ...
 *    \param filled Flag indicating if the pie should be filled (=1) or not
 * (=0).
 *
 *    \returns Returns 0 on success, -1 on failure.
 */
 ...
 * polygon.
 *    \param vy Vertex array containing Y coordinates of the points of the
 * polygon.
...
/*!
 *    \brief Internal helper qsort callback functions used in filled polygon
 * drawing.
 *
 *    \param a The surface to draw on.
 *    \param b Vertex array containing X coordinates of the points of the
 * polygon.
 *
 *    \returns Returns 0 if a==b, a negative number if a<b or a positive number
 * if a>b.
 */
 ...
 *
 *    Note: The last two parameters are optional; but are required for
 ...
 *    \param polyInts Preallocated, temporary vertex array used for sorting
 * vertices. Required for multithreaded operation; set to NULL otherwise.
 *    \param polyAllocated Flag indicating if temporary vertex array was
 * allocated. Required for multithreaded operation; set to NULL otherwise.
 *
 *    \returns Returns 0 on success, -1 on failure.
 */
 ...
 *
 *    Note: Standard filledPolygon function is calling multithreaded version
...
/*!
 *    \brief Draws a polygon filled with the given texture (Multi-Threading
 * Capable).
 *
 *    This operation use internally SDL_BlitSurface for lines of the source
 * texture. It supports
 *    alpha drawing.
 *
 ...
 *    The last two parameters are optional, but required for multithreaded
 * operation. When set to NULL, uses global static temp array.
 *
 ...
 *    \param texture_dx the offset of the texture relative to the screeen. if
 * you move the polygon 10 pixels
 *    to the left and want the texture to apear the same you need to increase
 * the texture_dx value
 *    \param texture_dy see texture_dx
 *    \param polyInts preallocated temp array storage for vertex sorting (used
 * for multi-threaded operation)
 *    \param polyAllocated flag indicating oif the temp array was allocated
 * (used for multi-threaded operation)
 *
 *    \returns Returns 0 on success, -1 on failure.
 */
...
/*!
 *    \brief Internal function to calculate bezier interpolator of data array
 * with ndata values at position 't'.
 *
 ...
 * between [0, ndata].
 *
 *    \returns Interpolated value at position t, value[0] when t<0, value[n-1]
 * when t>n.
 */
...
static void _murphyWideline(SDL_gfxMurphyIterator *m, Sint16 x, Sint16 y, Sint16 x2, Sint16 y2, Uint8 width, Uint8 miter)
{
    ...
    ang = atan((double) m->v / (double) m->u);  /* calc new initial point -
                                                 * offset both sides of ideal */
    sang = sin(ang);
    cang = cos(ang);
    ...
    for (q = 0; dd <= tk; q++) {    /* outer loop, stepping perpendicular to
                                     * line */
