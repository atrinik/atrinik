/*
 *
 *    SDL_rotozoom.c - rotozoomer, zoomer and shrinker for 32bit or 8bit
 * surfaces
 *
 *    LGPL (c) A. Schiffler
 *
 ...
 *    Averages color and alpha values values of src pixels to calculate dst
 * pixels.
 ...
 *    Averages color (brightness) values values of src pixels to calculate dst
 * pixels.
...
int _zoomSurfaceRGBA(SDL_Surface * src, SDL_Surface * dst, int flipx, int flipy, int smooth)
{
     ...
     */
    if ((sax = malloc((dst->w + 1) * sizeof(*sax))) == NULL) {
        return (-1);
    }

    if ((say = malloc((dst->h + 1) * sizeof(*say))) == NULL) {
        free(sax);
        return (-1);
    }
 ...
 *
 *    Rotates and zooms 32 bit RGBA/ABGR 'src' surface to 'dst' surface based on
 * the control
 *    parameters by scanning the destination surface and applying optionally
 * anti-aliasing
...
/*!
 *
 *    \brief Rotates and zooms 8 bit palette/Y 'src' surface to 'dst' surface
 * without smoothing.
 *
 *    Rotates and zooms 8 bit RGBA/ABGR 'src' surface to 'dst' surface based on
 * the control
 ...
 *    \param numClockwiseTurns Number of clockwise 90 degree turns to apply to
 * the source.
 *
...
SDL_Surface* rotateSurface90Degrees(SDL_Surface* src, int numClockwiseTurns)
{
    ...
    {
        /* Unfortunately SDL_BlitSurface cannot be used to make a copy of
         * the surface
...
/*!
 *    \brief Internal target surface sizing function for rotozooms with trig
 * result return.
 *
 ...
 *    \param zoomx The horizontal scaling factor.
 *    \param zoomy The vertical scaling factor.
 *    \param dstwidth The calculated width of the rotozoomed destination
 * surface.
 ...
 * surface.
 *    'angle' is the rotation in degrees and 'zoom' a scaling factor. If
 * 'smooth' is set
...
/*!
 *    \brief Rotates and zooms a surface with different horizontal and vertival
 * scaling factors and optional anti-aliasing.
 *
 *    Rotates and zooms a 32bit or 8bit 'src' surface to newly created 'dst'
 * surface.
 *    'angle' is the rotation in degrees, 'zoomx and 'zoomy' scaling factors. If
 * 'smooth' is set
 *    then the destination 32bit surface is anti-aliased. If the surface is not
 * 8bit
 *    or 32bit RGBA/ABGR it will be converted into a 32bit RGBA format on the
 ...
 *
 *    The minimum size of the target surface is 1. The input factors can be
 ...
 * zoomed target surface.
 *    \param dstheight Pointer to an integer to store the calculated height of
 * the zoomed target surface.
 */
...
SDL_Surface *shrinkSurface(SDL_Surface *src, int factorx, int factory)
{
    ...
    if (is32bit) {
        /*
         * Call the 32bit transformation routine to do the shrinking (using
         * alpha)
         */
