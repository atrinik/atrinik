...
static void widget_draw(widgetdata *widget)
{
	...
	if (widget->redraw)
	{
		...
		else if (strcmp(tmp->texture, "sphere") == 0)
		{
			...
			text_show_format(widget->surface, FONT_ARIAL11, WIDGET_BORDER_SIZE, WIDGET_BORDER_SIZE, COLOR_WHITE, TEXT_MARKUP, NULL, "<icon=stat_sphere_%s %d %d 1 0 %f>", widget->id, widget->w - WIDGET_BORDER_SIZE * 2, widget->h - WIDGET_BORDER_SIZE * 2, 4.0 + ((double) curr / (double) max));
