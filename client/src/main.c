...
void load_options_dat(void)
{
   ...
   */
   strcpy(options.metaserver, "atokar.is-a-geek.net");
...
Boolean game_status_chain(void)
{
        ...
        else if(GameStatus == GAME_STATUS_SETUP)
        {
            ...
            sprintf(buf,
                    "setup sound %d map2cmd 1 mapsize %dx%d darkness 1 facecache 1 skf %d|%x spf %d|%x bpf %d|%x stf %d|%x amf %d|%x hpf %d|%x",
					...
					srv_client_files[SRV_CLIENT_HFILES].len,srv_client_files[SRV_CLIENT_HFILES].crc
					);
...
int main(int argc, char *argv[])
{
	...
	sprintf(buf, "Welcome to Atrinik v%s", PACKAGE_VERSION);
