...
void walk_on_deep_swamp(object *op, object *victim)
{
	...
	{
		draw_info(COLOR_WHITE, victim, "You are down to your knees in the swamp.");
		op->stats.food = 1;
		...
		if (above->type == PLAYER && !QUERY_FLAG(above, FLAG_FLYING) && above->stats.hp >= 0)
		{
			...
			switch (op->stats.food)
			{
				case 1:
					if (rndm(0, 2) == 0)
					{
						draw_info(COLOR_WHITE, above, "You are down to your waist in the wet swamp.");
						op->stats.food = 2;
				...
				case 2:
					if (rndm(0, 2) == 0)
					{
						draw_info(COLOR_WHITE, above, "You are down to your NECK in the dangerous swamp.");
						op->stats.food = 3;
				...
				case 3:
					if (rndm(0, 4) == 0)
					{
						...
						{
							op->stats.food = 0;
							draw_info_flags_format(NDI_ALL, COLOR_WHITE, NULL, "%s disappeared into a swamp.", above->name);
							strcpy(CONTR(above)->killer, "drowning in a swamp");
