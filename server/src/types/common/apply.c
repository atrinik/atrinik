...
static bool object_apply_item_check_type(object *op, object *tmp) {
    ...
    if (op->type != CONTAINER && op->type == tmp->type &&
...
int object_apply_item(object *op, object *applier, int aflags) {
    SOFT_ASSERT_RC(op != NULL, OBJECT_METHOD_UNHANDLED, "Object to apply is NULL.");
    SOFT_ASSERT_RC(applier != NULL, OBJECT_METHOD_UNHANDLED, "Applier is NULL.");
    ...
    if (!QUERY_FLAG(op, FLAG_APPLIED)) {
        if (QUERY_FLAG(op, FLAG_QUEST_ITEM)) {
            draw_info(COLOR_WHITE, applier, "You feel you should turn it in first...");
            return OBJECT_METHOD_ERROR;
        }
    } else {
