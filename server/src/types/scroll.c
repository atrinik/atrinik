...
void apply_scroll(object *op, object *tmp)
{
	...
	{
		new_draw_info(0, COLOR_WHITE, op, "The scroll just doesn't make sense!");
		return;
	}
	...
	if (op->type == PLAYER)
	{
		...
		{
			new_draw_info(0, COLOR_WHITE, op, "You can read the scroll but you don't understand it.");
			return;
		}
	...
	new_draw_info_format(0, COLOR_WHITE, op, "The scroll of %s turns to dust.", spells[tmp->stats.sp].name);
