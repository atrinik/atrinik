...
void apply_poison(object *op, object *tmp)
{
	if (op->type == PLAYER)
	{
		...
		new_draw_info(0, COLOR_WHITE, op, "Yech! That tasted poisonous!");
...
void poison_more(object *op)
{
	...
	if (!op->stats.food)
	{
		...
		if (op->env->type == PLAYER)
		{
			...
			fix_player(op->env);
			new_draw_info(0, COLOR_WHITE, op->env, "You feel much better now.");
		}
	...
	{
		op->env->stats.food--;
		new_draw_info(0, COLOR_WHITE, op->env, "You feel very sick...");
	}
	...
	if (hit_player(op->env, op->stats.dam, op, AT_INTERNAL))
	{
		...
		for (i = 0; i < NUM_STATS; i++)
		{
			if (!(RANDOM() % 2) && get_attr_value(&op->stats, i) > -(MAX_STAT / 2))
			{
				...
				new_draw_info(0, COLOR_GRAY, op->env, lose_msg[i]);
			}
		}
