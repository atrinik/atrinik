 ...
 * result in a monster paths backtracking. It basically determines how
 * large a detour a monster will take from the direction path when
 * looking for a path to the player. The values are in the amount of
 * direction the deviation is. */
 ...
 * is no real path, it will just keep circling the chamber forever (this
 * could be a nice effect for monsters, but not for the function to get
 * stuck in). I think for the monsters, if max is reached and we return
 * the first direction the creature could move would result in the
 * circling behaviour. Unfortunately, this function is also used to
 * determined if the creature should cast a spell, so returning a
 * direction in that case is probably not a good thing. */
#define MAX_SPACES 50

/**
 * Returns the direction to the player, if valid. Returns 0 otherwise.
 *
 * Modified to verify there is a path to the player. Does this by
 * stepping towards player and if path is blocked then see if blockage is
 * close enough to player that direction to player is changed (ie zig or
 * zag). Continue zig zag until either reach player or path is blocked.
 * Thus, will only return 1 if there is a free path to player. Though
 * path may not be a straight line. Note that it will find player hiding
 * along a corridor at right angles to the corridor with the monster.
 *
 * Notes:
 *
 * -# With DETOUR_AMOUNT being 2, it should still go and find players
 *    hiding down corridors.
 * -# I think the old code was broken if the first direction the monster
 *    should move was blocked - the code would store the first direction
 *    without verifying that the player can actually move in that
 *    direction. The new code does not store anything in firstdir until
 *    we have verified that the monster can in fact move one space in
 *    that direction.
 * -# I'm not sure how good this code will be for moving multipart
 *    monsters, since only simple checks to blocked are being called,
 *    which could mean the monster is blocking itself.
...
int path_to_player(object *mon, object *pl, int mindiff)
{
	...
	m = mon->map;
	dir = rv.direction;
	/* Maybe we stand next to pl, init firstdir too */
	...
	while (diff > 1 && max > 0)
	{
		...
		if (arch_blocked(mon->arch, mon, m, x, y))
		{
			...
			if (rv.direction != dir)
			{
				/* OK - says direction should be different - let's reset
			...
			else
			{
				 ...
				 * more than -1/1 respectively will mean the monster
				 * takes bigger detour. Have to be careful about these
				 * values getting too big (3 or maybe 4 or higher) as
				 * the monster may just try stepping back and forth. */
				for (i = -DETOUR_AMOUNT; i <= DETOUR_AMOUNT; i++)
				{
					...
					/* Use lastdir here - otherwise, since the direction
					 * that the creature should move in may change, you
					 * could get infinite loops. ie, player is northwest,
					 * but monster can only move west, so it does that.
					 * It goes some distance, gets blocked, finds that it
					 * should move north, can't do that, but now finds it
					 * can move east, and gets back to its original
					 * point. lastdir contains the last direction the
					 * creature has successfully moved. */
				...
				/* Went through entire loop without finding a valid
		...
		if (diff <= 1)
		{
			/* Recalculate diff (distance) because we may not have
			 * actually headed toward player for entire distance. */
