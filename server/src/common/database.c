...
{
	int success = sqlite3_open("atrinik.db", db);
...
int db_prepare_format(sqlite3 *db, sqlite3_stmt **statement, const char *format, ...)
{
	...
	return db_prepare(db, buf, &*statement);
}
...
{
	/* We need a really huge buf for maps and player files. (~4 MB, should be enough) */
	char resultname[HUGE_BUF * MAX_BUF];

	/* Replace all 's with ''s. */
	replace(sql_input, "'", "''", resultname, sizeof(resultname));

	/* Copy the new result */
	strcpy(sql_input, resultname);
