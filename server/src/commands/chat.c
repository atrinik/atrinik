...
int command_dmsay(object *op, char *params)
{
	...
	for (pl = first_player; pl; pl = pl->next)
	{
		...
		{
			draw_info_format(NDI_PLAYER, COLOR_RED, pl->ob, "[DM Channel]: %s: %s", op->name, params);
		}
	}
...
int command_shout(object *op, char *params)
{
	...
	{
		draw_info(0, COLOR_WHITE, op, "You are no longer allowed to shout.");
		return 0;
	}

	LOG(llevChat, "Shout: %s: %s\n", query_name(op, NULL), params);
	draw_info_format(NDI_PLAYER | NDI_ALL | NDI_SHOUT, COLOR_ORANGE, NULL, "%s shouts: %s", op->name, params);
...
int command_tell(object *op, char *params)
{
	...
	if (!name)
	{
		draw_info(0, COLOR_WHITE, op, "Tell whom what?");
		return 1;
	}
	...
	if (!msg || *msg == '\0')
	{
		draw_info_format(0, COLOR_WHITE, op, "Tell %s what?", pl->ob->name);
		return 1;
	}
	...
	if (pl == CONTR(op))
	{
		draw_info(0, COLOR_WHITE, op, "You tell yourself the news. Very smart.");
		return 1;
	}

	if (pl->dm_stealth && !(QUERY_FLAG(op, FLAG_WIZ) || CONTR(op)->dm_stealth))
	{
		draw_info_format(NDI_PLAYER | NDI_TELL, COLOR_NAVY, pl->ob, "%s tells you (dm_stealth): %s", op->name, msg);
		draw_info(0, COLOR_WHITE, op, "No such player.");
	}
	else
	{
		draw_info_format(NDI_PLAYER, COLOR_NAVY, op, "You tell %s: %s", name, msg);
		draw_info_format(NDI_PLAYER | NDI_TELL, COLOR_NAVY, pl->ob, "%s tells you: %s", op->name, msg);
	}
	...
	if (!params)
	{
		...
		if (op->type == PLAYER && CONTR(op)->target_object != op && OBJECT_VALID(CONTR(op)->target_object, CONTR(op)->target_object_count) && CONTR(op)->target_object->name && (CONTR(op)->target_object->type != PLAYER || !CONTR(CONTR(op)->target_object)->dm_stealth || QUERY_FLAG(op, FLAG_WIZ) || CONTR(op)->dm_stealth) && !IS_INVISIBLE(CONTR(op)->target_object, op))
		{
			...
			if (get_rangevector(op, CONTR(op)->target_object, &rv, 0) && rv.distance <= 4)
			{
				...
				draw_info(NDI_PLAYER, COLOR_YELLOW, op, buf);

				if (CONTR(op)->target_object->type == PLAYER)
				{
					draw_info(NDI_PLAYER, COLOR_YELLOW, CONTR(op)->target_object, buf2);
				}
			...
			draw_info(0, COLOR_WHITE, op, "The target is not in range for this emote action.");
			return 0;
		}

		if (emotion == EMOTE_ME)
		{
			draw_info(0, COLOR_WHITE, op, "Usage: /me <emote to display>");
			return 0;
		}
		else if (emotion == EMOTE_MY)
		{
			draw_info(0, COLOR_WHITE, op, "Usage: /my <emote to display>");
			return 0;
		}
		...
		if (op->type == PLAYER)
		{
			...
			draw_info(NDI_PLAYER, COLOR_YELLOW, op, buf2);
		}
		else
		{
			...
			if (pl && (!pl->dm_stealth || QUERY_FLAG(op, FLAG_WIZ) || CONTR(op)->dm_stealth) && !IS_INVISIBLE(pl->ob, op))
			{
				...
				if (get_rangevector(op, pl->ob, &rv, 0) && rv.distance <= 4)
				{
					if (op->type == PLAYER)
					{
						...
						draw_info(NDI_PLAYER, COLOR_YELLOW, op, buf);
						draw_info(NDI_EMOTE | NDI_PLAYER, COLOR_YELLOW, pl->ob, buf2);
