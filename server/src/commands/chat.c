...
int command_dmsay(object *op, char *params)
{
	...
	for (pl = first_player; pl; pl = pl->next)
	{
		...
		{
			new_draw_info_format(NDI_PLAYER, COLOR_RED, pl->ob, "[DM Channel]: %s: %s", op->name, params);
		}
	}
...
int command_shout(object *op, char *params)
{
	...
	{
		new_draw_info(0, COLOR_WHITE, op, "You are no longer allowed to shout.");
		return 0;
	}

	LOG(llevChat, "Shout: %s: %s\n", query_name(op, NULL), params);
	new_draw_info_format(NDI_PLAYER | NDI_ALL | NDI_SHOUT, COLOR_ORANGE, NULL, "%s shouts: %s", op->name, params);
...
int command_tell(object *op, char *params)
{
	...
	if (!name)
	{
		new_draw_info(0, COLOR_WHITE, op, "Tell whom what?");
		return 1;
	}
	...
	if (!msg || *msg == '\0')
	{
		new_draw_info_format(0, COLOR_WHITE, op, "Tell %s what?", pl->ob->name);
		return 1;
	}
	...
	if (pl == CONTR(op))
	{
		new_draw_info(0, COLOR_WHITE, op, "You tell yourself the news. Very smart.");
		return 1;
	}

	if (pl->dm_stealth && !(QUERY_FLAG(op, FLAG_WIZ) || CONTR(op)->dm_stealth))
	{
		new_draw_info_format(NDI_PLAYER | NDI_TELL, COLOR_NAVY, pl->ob, "%s tells you (dm_stealth): %s", op->name, msg);
		new_draw_info(0, COLOR_WHITE, op, "No such player.");
	}
	else
	{
		new_draw_info_format(NDI_PLAYER, COLOR_NAVY, op, "You tell %s: %s", name, msg);
		new_draw_info_format(NDI_PLAYER | NDI_TELL, COLOR_NAVY, pl->ob, "%s tells you: %s", op->name, msg);
	}
	...
	if (!params)
	{
		...
		if (op->type == PLAYER && CONTR(op)->target_object != op && OBJECT_VALID(CONTR(op)->target_object, CONTR(op)->target_object_count) && CONTR(op)->target_object->name && (CONTR(op)->target_object->type != PLAYER || !CONTR(CONTR(op)->target_object)->dm_stealth || QUERY_FLAG(op, FLAG_WIZ) || CONTR(op)->dm_stealth) && !IS_INVISIBLE(CONTR(op)->target_object, op))
		{
			...
			if (get_rangevector(op, CONTR(op)->target_object, &rv, 0) && rv.distance <= 4)
			{
				...
				new_draw_info(NDI_PLAYER, COLOR_YELLOW, op, buf);

				if (CONTR(op)->target_object->type == PLAYER)
				{
					new_draw_info(NDI_PLAYER, COLOR_YELLOW, CONTR(op)->target_object, buf2);
				}

				new_info_map_except(NDI_EMOTE | NDI_PLAYER, COLOR_YELLOW, op->map, op->x, op->y, MAP_INFO_NORMAL, op, CONTR(op)->target_object, buf3);
				return 0;
			}

			new_draw_info(0, COLOR_WHITE, op, "The target is not in range for this emote action.");
			return 0;
		}

		if (emotion == EMOTE_ME)
		{
			new_draw_info(0, COLOR_WHITE, op, "Usage: /me <emote to display>");
			return 0;
		}
		else if (emotion == EMOTE_MY)
		{
			new_draw_info(0, COLOR_WHITE, op, "Usage: /my <emote to display>");
			return 0;
		}
		...
		if (op->type == PLAYER)
		{
			new_info_map_except(NDI_EMOTE | NDI_PLAYER, COLOR_YELLOW, op->map, op->x, op->y, MAP_INFO_NORMAL, op, op, buf);
			new_draw_info(NDI_PLAYER, COLOR_YELLOW, op, buf2);
		}
		else
		{
			new_info_map_except(NDI_EMOTE, COLOR_YELLOW, op->map, op->x, op->y, MAP_INFO_NORMAL, op, op, buf);
		}
	...
	else
	{
		...
		else
		{
			...
			if (op->type == PLAYER && strcmp(op->name, params) == 0)
			{
				...
				new_draw_info(NDI_PLAYER, COLOR_YELLOW, op, buf);
				new_info_map_except(NDI_EMOTE | NDI_PLAYER, COLOR_YELLOW, op->map, op->x, op->y, MAP_INFO_NORMAL, op, op, buf2);
				return 0;
			}
			...
			if (pl && (!pl->dm_stealth || QUERY_FLAG(op, FLAG_WIZ) || CONTR(op)->dm_stealth) && !IS_INVISIBLE(pl->ob, op))
			{
				...
				if (get_rangevector(op, pl->ob, &rv, 0) && rv.distance <= 4)
				{
					if (op->type == PLAYER)
					{
						...
						new_draw_info(NDI_PLAYER, COLOR_YELLOW, op, buf);
						new_draw_info(NDI_EMOTE | NDI_PLAYER, COLOR_YELLOW, pl->ob, buf2);
						new_info_map_except(NDI_EMOTE | NDI_PLAYER, COLOR_YELLOW, op->map, op->x, op->y, MAP_INFO_NORMAL, op, pl->ob, buf3);
					}
					else
					{
						emote_other(op, NULL, params, buf, sizeof(buf), buf2, sizeof(buf2), buf3, sizeof(buf3), emotion);
						new_draw_info(NDI_EMOTE | NDI_PLAYER, COLOR_YELLOW, pl->ob, buf2);
						new_info_map_except(NDI_EMOTE, COLOR_YELLOW, op->map, op->x, op->y, MAP_INFO_NORMAL, NULL, pl->ob, buf3);
					}
				}
			...
			{
				emote_other(op, NULL, params, buf, sizeof(buf), buf2, sizeof(buf2), buf3, sizeof(buf3), emotion);
				new_info_map_except(NDI_EMOTE, COLOR_YELLOW, op->map, op->x, op->y, MAP_INFO_NORMAL, NULL, NULL, buf3);
			}
		}
	}
