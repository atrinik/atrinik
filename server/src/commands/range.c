...
int command_cast_spell(object *op, char *params)
{
	...
	{
		new_draw_info(0, COLOR_WHITE, op, "You don't know any spells.");
		return 0;
	}

	if (params == NULL)
	{
		new_draw_info(0, COLOR_WHITE, op, "Cast which spell?");
		return 0;
	}
	...
	if (spnum == -1)
	{
		new_draw_info_format(0, COLOR_WHITE, op, "You don't know the spell %s.", params);
		return 0;
	}
