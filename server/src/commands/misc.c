...
void malloc_info(object *op)
{
	...
	draw_info_format(COLOR_WHITE, op, "Sizeof: object=%ld  player=%ld  map=%ld", (long) sizeof(object), (long) sizeof(player), (long) sizeof(mapstruct));

	dump_mempool_statistics(op, &sum_used, &sum_alloc);

	draw_info_format(COLOR_WHITE, op, "%4d active objects", count_active());

	draw_info_format(COLOR_WHITE, op, "%4d maps allocated:  %8d", nrofmaps, i = (nrofmaps * sizeof(mapstruct)));
	sum_alloc += i;
	sum_used += nrm * sizeof(mapstruct);

	draw_info_format(COLOR_WHITE, op, "%4d maps in memory:  %8d", nrm, mapmem);
	sum_alloc += mapmem;
	sum_used += mapmem;

	draw_info_format(COLOR_WHITE, op, "%4d archetypes:      %8d", anr, i = (anr * sizeof(archetype)));
	sum_alloc += i;
	sum_used += i;

	draw_info_format(COLOR_WHITE, op, "%4d animations:      %8d", anims, i = (anims * sizeof(Fontindex)));
	sum_alloc += i;
	sum_used += i;

	draw_info_format(COLOR_WHITE, op, "%4d spells:          %8d", NROFREALSPELLS, i = (NROFREALSPELLS * sizeof(spell_struct)));
	sum_alloc += i;
	sum_used += i;

	draw_info_format(COLOR_WHITE, op, "%4d treasurelists    %8d", tlnr, i = (tlnr * sizeof(treasurelist)));
	sum_alloc += i;
	sum_used += i;

	draw_info_format(COLOR_WHITE, op, "%4ld treasures        %8d", nroftreasures, i = (nroftreasures * sizeof(treasure)));
	sum_alloc += i;
	sum_used += i;

	draw_info_format(COLOR_WHITE, op, "%4ld artifacts        %8d", nrofartifacts, i = (nrofartifacts * sizeof(artifact)));
	sum_alloc += i;
	sum_used += i;

	draw_info_format(COLOR_WHITE, op, "%4ld artifacts strngs %8d", nrofallowedstr, i = (nrofallowedstr * sizeof(linked_char)));
	sum_alloc += i;
	sum_used += i;

	draw_info_format(COLOR_WHITE, op, "%4d artifactlists    %8d", alnr, i = (alnr * sizeof(artifactlist)));
	sum_alloc += i;
	sum_used += i;

	draw_info_format(COLOR_WHITE, op, "Total space allocated:%8d", sum_alloc);
	draw_info_format(COLOR_WHITE, op, "Total space used:     %8d", sum_used);
}
