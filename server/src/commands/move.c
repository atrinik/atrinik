...
int command_push_object(object *op, char *params)
{
	...
	{
		new_draw_info(0, COLOR_WHITE, op, "You are unable to push anything.");
		return 0;
	}
	...
	{
		new_draw_info(0, COLOR_WHITE, op, "You fail to push anything.");
		return 0;
	}
	...
	{
		new_draw_info_format(0, COLOR_WHITE, op, "You fail to push the %s.", query_name(tmp, NULL));
		return 0;
	}

	/* Now we move the player who was pushing the object. */
	move_ob(op, dir, op);
	new_draw_info_format(0, COLOR_WHITE, op, "You push the %s.", query_name(tmp, NULL));
