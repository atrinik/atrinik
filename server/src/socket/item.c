...
void LockItem(uint8 *data, int len, player *pl)
{
	...
	{
		draw_info(0, COLOR_WHITE, pl->ob, "You can't lock items outside your inventory!");
		return;
	}
...
void MarkItem(uint8 *data, int len, player *pl)
{
	...
	{
		draw_info_format(0, COLOR_WHITE, pl->ob, "Unmarked item %s.", query_name(op, NULL));
		pl->mark = NULL;
		pl->mark_count = -1;
	}
	else
	{
		draw_info_format(0, COLOR_WHITE, pl->ob, "Marked item %s.", query_name(op, NULL));
...
void esrv_move_object(object *pl, tag_t to, tag_t tag, long nrof)
{
	...
	if (!to)
	{
		...
		{
			draw_info(0, COLOR_WHITE, pl, "First remove all god-given items from this container!");
		}
		else if (QUERY_FLAG(pl, FLAG_INV_LOCKED))
		{
			draw_info(0, COLOR_WHITE, pl, "You can't drop a container with locked items inside!");
		}
		else
		{
	...
	if (env->type == CONTAINER && can_pick(pl, op) && sack_can_hold(pl, env, op, nrof))
	{
		...
		{
			draw_info(0, COLOR_WHITE, pl, "You can't store god-given items outside your inventory!");
		}
		else
		{
