...
static object *attempt_recipe(object *caster, object *cauldron, int ability, recipe *rp, int nbatches)
{
	...
	if (rp->keycode)
	{
		...
		if (tmp == NULL)
		{
			draw_info(0, COLOR_WHITE, caster, "You know the ingredients, but not the technique. Go learn how to do this recipe.");
			return NULL;
		}
	}
	...
	if ((item = make_item_from_recipe(cauldron, rp)) != NULL)
	{
		...
		{
			draw_info(0, COLOR_WHITE, caster, "Nothing happened.");
		}
		else
		{
			draw_info_format(0, COLOR_WHITE, caster, "The %s %s.", cauldron->name, cauldron_sound());
		}
	}
...
static void alchemy_failure_effect(object *op, object *cauldron, recipe *rp, int danger)
{
	...
	if (level < 25)
	{
		...
		remove_contents(cauldron->inv, item);
		draw_info_format(0, COLOR_WHITE, op, "The %s %s.", cauldron->name, cauldron_sound());
		return;
	}
	/* Make tained item. */
	else if (level < 40)
	...
	else if (level < 50)
	{
		...
		switch (rndm(0, 2))
		{
			case 0:
				...
				draw_info_format(0, COLOR_WHITE, op, "The %s creates a bomb!", cauldron->name);
				break;
			...
			default:
				...
				draw_info_format(0, COLOR_WHITE, op, "The %s erupts in flame!", cauldron->name);
				break;
		}
	...
	else if (level < 100)
	{
		...
		{
			remove_contents(cauldron->inv, NULL);
			draw_info_format(0, COLOR_WHITE, op, "Your %s turns darker then makes a gulping sound!", cauldron->name);
		}
		else
		{
			draw_info_format(0, COLOR_WHITE, op, "Your %s becomes darker.", cauldron->name);
		}
	...
	{
		draw_info(0, COLOR_WHITE, op, "You unwisely release potent forces!");
		remove_contents(cauldron->inv, NULL);
...
int use_alchemy(object *op)
{
	...
	if (unpaid_cauldron)
	{
		draw_info_format(0, COLOR_WHITE, op, "You must pay for your %s first!", query_base_name(unpaid_cauldron, NULL));
	}
	else if (unpaid_item)
	{
		draw_info_format(0, COLOR_WHITE, op, "You must pay for your %s first!", query_base_name(unpaid_item, NULL));
	}
