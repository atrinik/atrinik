...
static void remove_confusion(object *op)
{
	...
	if (op->env)
	{
		...
		{
			new_draw_info(0, COLOR_WHITE, op->env, "You regain your senses.");
		}
	}
...
static void execute_wor(object *op)
{
	...
	if (op != NULL)
	{
		...
		{
			new_draw_info(0, COLOR_WHITE, op, "You feel something fizzle inside you.");
		}
		else
		{
			enter_exit(op, wor);
			...
			if (op->type == LIGHT_APPLY)
			{
				...
				if (op->other_arch == NULL)
				{
					...
					if (op->env)
					{
						...
						if (op->env->type == PLAYER)
						{
							new_draw_info_format(0, COLOR_WHITE, op->env, "The %s burnt out.", query_name(op, NULL));
							op->glow_radius = 0;
