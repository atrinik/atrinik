 ...
 * (anything that is type BOOK).
 * @param pl Player writing.
 * @param item Book to write into.
 * @param msg Message to write.
 * @return Experience gained for writing. */
static int write_note(object *pl, object *item, const char *msg)
{
	...
	if (!msg)
	{
		new_draw_info_format(NDI_UNIQUE, 0, pl," No message to write!\nUsage: /use_skill %s <message>", skills[SK_INSCRIPTION].name);
		return 0;
	}
...
/**
 * This routine allows players to inscribe spell scrolls of spells which
 * they know. Backfire effects are possible with the severity of the
 * backlash correlated with the difficulty of the scroll that is
 * attempted.
 * @param pl Player writing a scroll.
 * @param scroll Object to write into.
 * @return Experience gained. */
static int write_scroll(object *pl, object *scroll)
{
	...
	if (pl->path_denied & spells[chosen_spell].path)
	{
		new_draw_info_format(NDI_UNIQUE, 0, pl, "The simple idea of writing a scroll of %s makes you sick!", spells[chosen_spell].name);
		return 0;
	}

	/* If there is a spell already on the scroll then player could easily
	 * accidentally read it while trying to write the new one. Give
	 * player a 50% chance to overwrite spell at their own level. */
 ...
 * @param pl Player writing.
 * @param params Message to inscribe.
 * @return Experience gained for using the skill. */
int write_on_item(object *pl, const char *params)
{
	...
	{
		new_draw_info_format(NDI_UNIQUE, 0, pl, "You have no %s to write %s.", msgtype == BOOK ? "book" : "scroll", msgtype == BOOK ? "on" : "your spell down");
		return 0;
	}
