...
sint64 find_traps(object *pl, int level)
{
	...
	if (!suc)
	{
		draw_info(0, COLOR_WHITE, pl, "You can't detect any trap here.");
	}
	else if (suc == 2)
	{
		draw_info(0, COLOR_WHITE, pl, "You detect trap signs!");
	}
...
sint64 remove_trap(object *op)
{
	...
	draw_info(0, COLOR_WHITE, op, "There is no trap to remove nearby.");
	return 0;
}
...
else
{
	...
	if (QUERY_FLAG(tmp, FLAG_APPLIED))
	{
		...
		{
			draw_info_format(0, COLOR_WHITE, op, "You can't throw %s.", query_base_name(tmp, NULL));
			return NULL;
		}
		else if (QUERY_FLAG(tmp, FLAG_STARTEQUIP))
		{
			draw_info(0, COLOR_WHITE, op, "You can't throw god-given item!");
			return NULL;
		}
		/* If cursed or damned, we can't unapply it - no throwing. */
		else if (QUERY_FLAG(tmp, FLAG_CURSED) || QUERY_FLAG(tmp, FLAG_DAMNED))
		{
			draw_info_format(0, COLOR_WHITE, op, "The %s sticks to your hand!", query_base_name(tmp, NULL));
			return NULL;
		}
	...
	else
	{
		...
		{
			draw_info_format(0, COLOR_WHITE, op, "You must apply the %s first.", query_base_name(tmp, NULL));
			return NULL;
		}
	}
...
void do_throw(object *op, object *toss_item, int dir)
{
	...
	if (!throw_ob)
	{
		if (op->type == PLAYER)
		{
			draw_info(0, COLOR_WHITE, op, "You have nothing to throw.");
		}
	...
	if (QUERY_FLAG(throw_ob, FLAG_STARTEQUIP))
	{
		if (op->type == PLAYER)
		{
			draw_info(0, COLOR_WHITE, op, "The gods won't let you throw that.");
		}
	...
	{
		draw_info_format(0, COLOR_WHITE, op, "You can't throw %s.\n", query_base_name(throw_ob, NULL));
		return;
	}
	...
	if (!dir || wall(op->map, op->x + freearr_x[dir], op->y + freearr_y[dir]))
	{
		...
		if (op->type == PLAYER)
		{
			if (!dir)
			{
				draw_info_format(0, COLOR_WHITE, op, "You drop %s at the ground.", query_name(throw_ob, NULL));
			}
			else
			{
