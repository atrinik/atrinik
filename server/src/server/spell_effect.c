...
int recharge(object *op)
{
	...
	{
		draw_info(0, COLOR_RED, op, "You need to mark the wand you want to recharge.");
		return 0;
	}

	if (wand->stats.sp < 0 || wand->stats.sp >= NROFREALSPELLS || !spells[wand->stats.sp].charges)
	{
		draw_info_format(0, COLOR_RED, op, "The %s cannot be recharged.", query_name(wand, NULL));
		return 0;
	}

	if (!(rndm(0, 6)))
	{
		draw_info_format(0, COLOR_WHITE, op, "The %s vibrates violently, then explodes!", query_name(wand, NULL));
	...
	draw_info_format(0, COLOR_WHITE, op, "The %s glows with power.", query_name(wand, NULL));
...
int cast_create_food(object *op, object *caster, int dir, const char *stringarg)
{
	...
	if (!at)
	{
		draw_info(0, COLOR_WHITE, op, "You don't have enough experience to create any food.");
		return 0;
	}
...
int probe(object *op)
{
	...
	for (tmp = get_map_ob(op->map, op->x, op->y); tmp != NULL; tmp = tmp->above)
	{
		if (IS_LIVE(tmp))
		{
			...
			{
				draw_info_format(0, COLOR_WHITE, op->owner, "Your probe analyzes %s.", tmp->name);
...
int cast_wor(object *op, object *caster)
{
	...
	{
		draw_info(0, COLOR_WHITE, op, "Something blocks your spell.");
		return 0;
	}
	...
	draw_info(0, COLOR_WHITE, op, "You feel a force starting to build up inside you.");
...
int cast_create_town_portal(object *op)
{
	...
	{
		draw_info(0, COLOR_NAVY, op, "You can't cast that here.\n");
		return 0;
	}
	...
	if (force == NULL)
	{
		...
		update_ob_speed(dummy);
		insert_ob_in_ob(dummy, op);
		draw_info(0, COLOR_NAVY, op, "You fix this place in your mind.\nYou feel you are able to come here from anywhere.");
		return 1;
	}
	...
	if (exitmap == NULL)
	{
		draw_info(0, COLOR_NAVY, op, "Something strange happened.\nYou can't remember where to go?!");
		FREE_AND_CLEAR_HASH(exitpath);
		return 1;
	}
	...
	draw_info(0, COLOR_NAVY, op, "You see air moving and showing you the way home.");
	FREE_AND_CLEAR_HASH(exitpath);
	return 1;
}
...
int cast_heal_around(object *op, int level, int type)
{
	...
	switch (type)
	{
		...
		case SP_PARTY_HEAL:
		{
			...
			{
				draw_info(0, COLOR_WHITE, op, "You need to be in a party to cast this spell.");
				return 0;
			}
...
int cast_heal(object *op, int level, object *target, int spell_type)
{
	...
	switch (spell_type)
	{
		...
		case SP_CURE_POISON:
			...
			{
				draw_info_format(0, COLOR_WHITE, target, "%s casts cure poison on you!", op->name ? op->name : "Someone");
			}

			if (op != target && op->type == PLAYER)
			{
				draw_info_format(0, COLOR_WHITE, op, "You cast cure poison on %s!", target->name ? target->name : "someone");
			}
			...
			if (success)
			{
				...
				{
					draw_info(0, COLOR_WHITE, target, "Your body feels cleansed.");
				}

				if (op != target && op->type == PLAYER)
				{
					draw_info_format(0, COLOR_WHITE, op, "%s's body seems cleansed.", target->name ? target->name : "Someone");
				}
			}
			else
			{
				if (target->type == PLAYER)
				{
					draw_info(0, COLOR_WHITE, target, "You are not poisoned.");
				}

				if (op != target && op->type == PLAYER)
				{
					draw_info_format(0, COLOR_WHITE, op, "%s is not poisoned.", target->name ? target->name : "Someone");
				}
			}
		...
		case SP_CURE_CONFUSION:
			...
			if (op != target && target->type == PLAYER)
			{
				draw_info_format(0, COLOR_WHITE, target, "%s casts cure confusion on you!", op->name ? op->name : "Someone");
			}

			if (op != target && op->type == PLAYER)
			{
				draw_info_format(0, COLOR_WHITE, op, "You cast cure confusion on %s!", target->name ? target->name : "someone");
			}
			...
			if (success)
			{
				if (target->type == PLAYER)
				{
					draw_info(0, COLOR_WHITE, target, "Your mind feels clearer.");
				}

				if (op != target && op->type == PLAYER)
				{
					draw_info_format(0, COLOR_WHITE, op, "%s's mind seems clearer.", target->name ? target->name : "Someone");
				}
			}
			else
			{
				if (target->type == PLAYER)
				{
					draw_info(0, COLOR_WHITE, target, "You are not confused.");
				}

				if (op != target && op->type == PLAYER)
				{
					draw_info_format(0, COLOR_WHITE, op, "%s is not confused.", target->name ? target->name : "Someone");
				}
			}
		...
		case SP_MINOR_HEAL:
			...
			if (op->type == PLAYER)
			{
				if (heal > 0)
				{
					draw_info_format(0, COLOR_WHITE, op, "The prayer heals %s for %d hp!", op == target ? "you" : (target ? target->name : "NULL"), heal);
				}
				else
				{
					draw_info(0, COLOR_WHITE, op, "The healing prayer fails!");
				}
			}
			...
			if (op != target && target->type == PLAYER)
			{
				if (heal > 0)
				{
					draw_info_format(0, COLOR_WHITE, target, "%s casts minor healing on you healing %d hp!", op->name, heal);
				}
				else
				{
					draw_info_format(0, COLOR_WHITE, target, "%s casts minor healing on you but it fails!", op->name);
				}
			}
		...
		case SP_GREATER_HEAL:
			...
			if (op != target && target->type == PLAYER)
			{
				if (heal > 0)
				{
					draw_info_format(0, COLOR_WHITE, target, "%s casts greater healing on you healing %d hp!", op->name, heal);
				}
				else
				{
					draw_info_format(0, COLOR_WHITE, target, "%s casts greater healing on you but it fails!", op->name);
				}
			}
...
int cast_change_attr(object *op, object *caster, object *target, int spell_type)
{
	...
	for (tmp2 = tmp->inv; tmp2 != NULL; tmp2 = tmp2->below)
	{
		if (tmp2->type == FORCE)
		{
			if (tmp2->value == spell_type)
			{
				...
				force = tmp2;
				is_refresh = 1;
				draw_info(0, COLOR_WHITE, op, "You recast the spell while in effect.");
			}
		}
	}
	...
	switch (spell_type)
	{
		case SP_STRENGTH:
			...
			if (tmp->type != PLAYER)
			{
				if (op->type == PLAYER)
				{
					draw_info(0, COLOR_WHITE, op, "You can't cast this kind of spell on your target.");
				}
			...
			{
				draw_info_format(0, COLOR_WHITE, tmp, "%s casts strength on you!", op->name ? op->name : "Someone");
			}
			...
			if (force->stats.Str < 2)
			{
				...
				{
					draw_info_format(0, COLOR_WHITE, op, "%s gets stronger.", tmp->name ? tmp->name : "Someone");
				}
			}
			else
			{
				msg_flag = 0;
				draw_info(0, COLOR_WHITE, tmp, "You don't grow stronger but the spell is refreshed.");

				if (op->type == PLAYER && op != tmp)
				{
					draw_info_format(0, COLOR_WHITE, op, "%s doesn't grow stronger but the spell is refreshed.", tmp->name ? tmp->name : "Someone");
				}
			}
	...
	if (i)
	{
		draw_info_format(0, COLOR_WHITE, op, "Your protection to %s grows.", attack_name[i]);
...
int create_bomb(object *op, object *caster, int dir, int spell_type)
{
	...
	{
		draw_info(0, COLOR_WHITE, op, "There is something in the way.");
		return 0;
	}
...
int remove_depletion(object *op, object *target)
{
	...
	if (target->type != PLAYER)
	{
		...
		if (op->type == PLAYER)
		{
			draw_info_format(0, COLOR_WHITE, op, "You cast depletion on %s.", query_base_name(target, NULL));
			draw_info(0, COLOR_WHITE, op, "There is no depletion.");
		}
	...
	if (op != target)
	{
		...
		{
			draw_info_format(0, COLOR_WHITE, target, "%s casts remove depletion on you.", query_base_name(op, NULL));
		}
	}
	...
	if ((depl = present_arch_in_ob(at, target)) != NULL)
	{
		for (i = 0; i < NUM_STATS; i++)
		{
			...
			{
				success++;
				draw_info(0, COLOR_WHITE, target, restore_msg[i]);
			}
		}
	...
	if (op != target && op->type == PLAYER)
	{
		if (success)
		{
			draw_info(0, COLOR_WHITE, op, "Your prayer removes some depletion.");
		}
		else
		{
			draw_info(0, COLOR_WHITE, op, "There is no depletion.");
	...
	{
		draw_info(0, COLOR_WHITE, target, "There is no depletion.");
	}
...
int remove_curse(object *op, object *target, int type, int src)
{
	...
	if (op != target)
	{
		if (op->type == PLAYER)
		{
			draw_info_format(0, COLOR_WHITE, op, "You cast remove %s on %s.", type == SP_REMOVE_CURSE ? "curse" : "damnation", query_base_name(target, NULL));
		}
		else if (target->type == PLAYER)
		{
			draw_info_format(0, COLOR_WHITE, target, "%s casts remove %s on you.", query_base_name(op, NULL), type == SP_REMOVE_CURSE ? "curse" : "damnation");
		}
	}
	...
	for (tmp = target->inv; tmp; tmp = tmp->below)
	{
		if ((src == CAST_NPC || QUERY_FLAG(tmp, FLAG_APPLIED)) && (QUERY_FLAG(tmp, FLAG_CURSED) || (type == SP_REMOVE_DAMNATION && QUERY_FLAG(tmp, FLAG_DAMNED))))
		{
			...
			else
			{
				if (target->type == PLAYER)
				{
					draw_info_format(0, COLOR_WHITE, target, "The %s's curse is stronger than the prayer!", query_base_name(tmp, NULL));
				}
				else if (op != target && op->type == PLAYER)
				{
					draw_info_format(0, COLOR_WHITE, op, "The %s's curse of %s is stronger than your prayer!", query_base_name(tmp, NULL), query_base_name(target, NULL));
				}
			}
		}
	}
	...
	if (op != target && op->type == PLAYER)
	{
		if (success)
		{
			draw_info(0, COLOR_WHITE, op, "Your prayer removes some curses.");
		}
		else
		{
			draw_info_format(0, COLOR_WHITE, op, "%s's items seem uncursed.", query_base_name(target, NULL));
		}
	}
	...
	if (target->type == PLAYER)
	{
		if (success)
		{
			draw_info(0, COLOR_WHITE, target, "You feel like someone is helping you.");
		}
		else
		{
			if (src == CAST_NORMAL)
			{
				draw_info(0, COLOR_WHITE, target, "You are not using any cursed items.");
			}
			else
			{
				draw_info(0, COLOR_WHITE, target, "You hear maniacal laughter in the distance.");
			}
		}
	}
...
int do_cast_identify(object *tmp, object *op, int mode, int *done, int level)
{
	...
	if (level < tmp->level)
	{
		if (op->type == PLAYER)
		{
			draw_info_format(0, COLOR_WHITE, op, "The %s is too powerful for this identify!", query_base_name(tmp, NULL));
		}
	}
	else
	{
		...
		if (op->type == PLAYER)
		{
			draw_info_format(0, COLOR_WHITE, op, "You have %s.", long_desc(tmp, NULL));

			if (tmp->msg)
			{
				draw_info(0, COLOR_WHITE, op, "The item has a story:");
...
int cast_identify(object *op, int level, object *single_ob, int mode)
{
	...
	{
		draw_info(0, COLOR_WHITE, op, "You can't reach anything unidentified in your inventory.");
	}
...
int cast_consecrate(object *op)
{
	...
	if (!god)
	{
		draw_info(0, COLOR_WHITE, op, "You can't consecrate anything if you don't worship a god!");
		return 0;
	}
	...
	for (tmp = op->below; tmp; tmp = tmp->below)
	{
		...
		if (tmp->type == HOLY_ALTAR)
		{
			...
			if (tmp->level > SK_level(op))
			{
				draw_info_format(0, COLOR_WHITE, op, "You are not powerful enough to reconsecrate the %s.", tmp->name);
				return 0;
			}
			else if (tmp->other_arch == god->arch)
			{
				draw_info_format(0, COLOR_WHITE, op, "That altar is already consecrated to %s.", god->name);
				return 0;
			}
			else
			{
				...
				draw_info_format(0, COLOR_WHITE, op, "You consecrated the altar to %s!", god->name);
				return 1;
			}
		}
	}

	draw_info(0, COLOR_WHITE, op, "You are not standing over an altar!");
	return 0;
}
...
int finger_of_death(object *op, object *target)
{
	...
	{
		draw_info_format(0, COLOR_WHITE, op, "The %s looks stronger!", query_name(target, NULL));
		target->stats.hp = target->stats.maxhp;
	...
	{
		draw_info_format(0, COLOR_ORANGE, op, "Your finger of death misses %s!", target->name);
		remove_ob(hitter);
		return 1;
	}
...
int cast_cause_disease(object *op, object *caster, int dir, archetype *disease_arch, int type)
{
	...
	for (i = 0; i < 5; i++)
	{
		...
		for (walk = GET_MAP_OB(m, xt, yt); walk; walk = walk->above)
		{
			...
			{
				draw_info_format(0, COLOR_WHITE, op, "You inflict %s on %s!", disease->name, walk->name);
				return 1;
			}
		}
	...
	draw_info(0, COLOR_WHITE, op, "No one caught anything!");
	return 0;
}
...
int cast_transform_wealth(object *op)
{
	...
	if (!marked)
	{
		draw_info(0, COLOR_WHITE, op, "You need to mark an object to cast this spell.");
		return 0;
	}
	...
	{
		draw_info(0, COLOR_WHITE, op, "You can only cast this spell on wealth objects.");
		return 0;
	}
	...
	{
		draw_info_format(0, COLOR_WHITE, op, "You don't see a way to transform %s.", query_name(marked, op));
		return 0;
	}
	...
	insert_coins(op, val);
	draw_info_format(0, COLOR_WHITE, op, "You transform %s into %s.", query_name(marked, op), cost_string_from_value(val));
	return 1;
}
