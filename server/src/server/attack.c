	...
	else
	{
		if (hitter->type != ARROW)
		{
			if (hitter->type == PLAYER)
			{
				...
				draw_info_format(0, COLOR_ORANGE, hitter, "You miss %s!", op->name);

				if (op->type == PLAYER)
				{
					draw_info_format(0, COLOR_PURPLE, op, "%s misses you!", hitter->name);
				}
			}
			else
			{
...
int hit_player(object *op, int dam, object *hitter, int type)
{
	...
	{
		draw_info(0, COLOR_WHITE, op, "Your praying is disrupted.");
		CONTR(op)->praying = 0;
		...
		case ATNR_BLIND:
		{
			...
			if (op->speed && (QUERY_FLAG(op, FLAG_MONSTER) || op->type == PLAYER) && !(rndm(0, (attacknum == ATNR_SLOW ? 6 : 3) - 1)) && ((rndm(1, 20) + op->protection[attacknum] / 10) < savethrow[level_diff]))
			{
				if (attacknum == ATNR_CONFUSION)
				{
					if (hitter->type == PLAYER)
					{
						draw_info_format(0, COLOR_ORANGE, hitter, "You confuse %s!", op->name);
					}

					if (op->type == PLAYER)
					{
						draw_info_format(0, COLOR_PURPLE, op, "%s confused you!", hitter->name);
					}
				...
				else if (attacknum == ATNR_SLOW)
				{
					if (hitter->type == PLAYER)
					{
						draw_info_format(0, COLOR_ORANGE, hitter, "You slow %s!", op->name);
					}

					if (op->type == PLAYER)
					{
						draw_info_format(0, COLOR_PURPLE, op, "%s slowed you!", hitter->name);
					}
				...
				else if (attacknum == ATNR_PARALYZE)
				{
					if (hitter->type == PLAYER)
					{
						draw_info_format(0, COLOR_ORANGE, hitter, "You paralyze %s!", op->name);
					}

					if (op->type == PLAYER)
					{
						draw_info_format(0, COLOR_PURPLE, op, "%s paralyzed you!", hitter->name);
					}
				...
				else if (attacknum == ATNR_BLIND && !QUERY_FLAG(op, FLAG_UNDEAD))
				{
					if (hitter->type == PLAYER)
					{
						draw_info_format(0, COLOR_ORANGE, hitter, "You blind %s!", op->name);
					}

					if (op->type == PLAYER)
					{
						draw_info_format(0, COLOR_PURPLE, op, "%s blinded you!", hitter->name);
					}
...
static void send_attack_msg(object *op, object *hitter, int attacknum, int dam, int damage)
{
	...
	if (op->type == PLAYER)
	{
		draw_info_format(0, COLOR_PURPLE, op, "%s hit you for %d (%d) damage.", hitter->name, dam, dam - damage);
	}

	if (hitter->type == PLAYER || ((hitter = get_owner(hitter)) && hitter->type == PLAYER))
	{
		draw_info_format(0, COLOR_ORANGE, hitter, "You hit %s for %d (%d) with %s.", op->name, dam, dam - damage, attacknum == ATNR_INTERNAL ? orig_hitter->name : attack_name[attacknum]);
	}
}
...
static void share_kill_exp_one(object *op, sint64 exp, object *skill)
{
	...
	{
		draw_info(0, COLOR_WHITE, op, "Your enemy wasn't worth any experience to you.");
	}
}
...
int kill_object(object *op, int dam, object *hitter, int type)
{
	...
	if (owner->type == PLAYER)
	{
		if (owner != hitter)
		{
			draw_info_format(0, COLOR_WHITE, owner, "You killed %s with %s.", query_name(op, NULL), query_name(hitter, NULL));
		}
		else
		{
			draw_info_format(0, COLOR_WHITE, owner, "You killed %s.", query_name(op, NULL));
		}
	...
	{
		draw_info(0, COLOR_WHITE, owner, "Your foe has fallen!\nVICTORY!!!");
	}
	...
	if (op->type == PLAYER)
	{
		...
		if (get_owner(hitter))
		{
			draw_info_format(NDI_ALL, COLOR_WHITE, NULL, "%s killed %s with %s%s.", hitter->owner->name, query_name(op, NULL), query_name(hitter, NULL), battleg ? " (duel)" : "");
		}
		else
		{
			draw_info_format(NDI_ALL, COLOR_WHITE, NULL, "%s killed %s%s.", hitter->name, op->name, battleg ? " (duel)" : "");
		}
		...
		else
		{
			...
			if (op->type == PLAYER)
			{
				...
				if (hitter->type == POISON)
				{
					...
					draw_info(0, COLOR_WHITE, op, "You suddenly feel very ill.");
				}
				/* We have hit with weapon or something */
				else
				{
					draw_info_format(0, COLOR_WHITE, op, "%s has poisoned you!", query_name(hitter, NULL));
			...
			else
			{
				...
				else
				{
					...
					if (hitter->type == PLAYER)
					{
						draw_info_format(0, COLOR_WHITE, hitter, "You poisoned %s!", query_name(op, NULL));
					}
					else if (get_owner(hitter) && hitter->owner->type == PLAYER)
					{
						draw_info_format(0, COLOR_WHITE, hitter->owner, "%s poisoned %s!", query_name(hitter, NULL), query_name(op, NULL));
					}
...
static void slow_living(object *op)
{
	...
	if ((tmp = present_arch_in_ob(at, op)) == NULL)
	{
		...
		draw_info(0, COLOR_WHITE, op, "The world suddenly moves very fast!");
	}
	else
	{
		tmp->stats.food++;
...
void confuse_living(object *op)
{
	...
	{
		draw_info(0, COLOR_WHITE, op, "You suddenly feel very confused!");
	}
...
static void blind_living(object *op, object *hitter, int dam)
{
	...
	if (!tmp)
	{
		...
		draw_info_format(0, COLOR_WHITE, owner, "Your attack blinds %s!", query_name(op, NULL));
	}
