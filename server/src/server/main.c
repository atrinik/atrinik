...
static int shutdown_timer_check(void)
{
	...
	{
		draw_info_type_format(CHAT_TYPE_CHAT, "[Server]", COLOR_GREEN, NULL, "Server will shut down in %02"FMT64U":%02"FMT64U" minutes.", (uint64) ((shutdown_time - pticks) / (1000000 / max_time) / 60), (uint64) ((shutdown_time - pticks) / (1000000 / max_time) % 60));
	}
