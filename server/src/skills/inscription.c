...
static int inscribe_book(object *op, const char *msg, object *marked)
{
	...
	if (!msg || *msg == '\0')
	{
		draw_info(0, COLOR_WHITE, op, "No message to write. Usage:\n/use_skill inscription <message>");
		return 0;
	}
	...
	{
		draw_info(0, COLOR_WHITE, op, "Trying to cheat now are we?");
	...
	{
		draw_info_format(0, COLOR_WHITE, op, "Your message won't fit in the %s.", query_short_name(marked, op));
		return 0;
	}
	...
	draw_info_format(0, COLOR_WHITE, op, "You write in the %s.", query_short_name(marked, op));
...
int skill_inscription(object *op, const char *params)
{
	...
	{
		draw_info(0, COLOR_WHITE, op, "You must learn to read before you can write!");
		return 0;
	}
	...
	if (!skill_item)
	{
		draw_info(0, COLOR_WHITE, op, "You need to apply a writing pen to use this skill.");
		return 0;
	}

	if (skill_item->stats.sp != SK_INSCRIPTION)
	{
		draw_info_format(0, COLOR_WHITE, op, "The %s cannot be used with this skill.", query_short_name(skill_item, NULL));
		return 0;
	}

	if (QUERY_FLAG(op, FLAG_BLIND) && !QUERY_FLAG(op, FLAG_WIZ))
	{
		draw_info(0, COLOR_WHITE, op, "You are unable to write while blind.");
		return 0;
	}
	...
	if (!marked)
	{
		draw_info(0, COLOR_WHITE, op, "You don't have any marked item to write on.");
		return 0;
	}
	...
	draw_info_format(0, COLOR_WHITE, op, "You can't write on the %s.", query_short_name(marked, NULL));
	return 0;
}
