...
static int builder_floor(object *op, object *new_floor, int x, int y)
{
	...
	for (tmp = GET_MAP_OB_LAYER(op->map, x, y, LAYER_FLOOR - 1); tmp && tmp->layer == LAYER_FLOOR; tmp = tmp->above)
	{
		if (tmp->type == FLOOR || QUERY_FLAG(tmp, FLAG_IS_FLOOR))
		{
			...
			{
				draw_info(0, COLOR_WHITE, op, "You feel too lazy to redo the exact same floor.");
				return 0;
			}
	...
	draw_info(0, COLOR_WHITE, op, "You change the floor to better suit your tastes.");
...
static int builder_item(object *op, object *new_item, int x, int y)
{
	...
	{
		draw_info_format(0, COLOR_WHITE, op, "Something is blocking you from building the %s on that square.", query_name(new_item, NULL));
		return 0;
	}
	...
	else if (new_item->type == WALL)
	{
		...
		if (!w || !wall_ob)
		{
			draw_info_format(0, COLOR_WHITE, op, "The %s can only be built on top of a wall.", query_name(new_item, NULL));
			return 0;
		}
		else if (wall_ob->above && wall_ob->above->type == WALL)
		{
			draw_info_format(0, COLOR_WHITE, op, "You first need to remove the %s before building on top of that wall again.", query_name(wall_ob->above, NULL));
			return 0;
		}
	}
	...
	if (!floor)
	{
		draw_info(0, COLOR_WHITE, op, "This square has no floor, you can't build here.");
		return 0;
	}
	...
	if (new_item->type == SIGN)
	{
		...
		{
			draw_info(0, COLOR_WHITE, op, "You need to put a book with your message (or custom name) on the floor.");
			return 0;
		}
	...
	draw_info_format(0, COLOR_WHITE, op, "You build the %s.", query_name(new_item, NULL));
...
static int builder_wall(object *op, object *new_wall, int x, int y)
{
	...
	if (wall_ob)
	{
		...
		{
			draw_info(0, COLOR_WHITE, op, "You don't see a way to redecorate that wall.");
			return 0;
		}

		if (!strncmp(wall_ob->arch->name, wall_name, strlen(wall_name)))
		{
			draw_info(0, COLOR_WHITE, op, "You feel too lazy to redo the exact same wall.");
			return 0;
		}
	}
	...
	if (wall_ob)
	{
		...
		draw_info(0, COLOR_WHITE, op, "You redecorate the wall to better suit your tastes.");
	}
	...
	else
	{
		...
		draw_info(0, COLOR_WHITE, op, "You build a wall.");
	}
...
static int builder_window(object *op, int x, int y)
{
	...
	if (!wall_ob)
	{
		draw_info(0, COLOR_WHITE, op, "There is no wall there.");
		return 0;
	}

	if (!wall_split_orientation(wall_ob, wall_name, sizeof(wall_name), orientation, sizeof(orientation)))
	{
		draw_info(0, COLOR_WHITE, op, "You don't see a way to build a window in that wall.");
		return 0;
	}
	else if (!strcmp(orientation, "_1") && !strcmp(orientation, "_3"))
	{
		draw_info(0, COLOR_WHITE, op, "You cannot build a window in that wall.");
		return 0;
	}
	...
	draw_info(0, COLOR_WHITE, op, "You build a window in the wall.");
	return 1;
}
...
static void construction_builder(object *op, int x, int y)
{
	...
	if (!material)
	{
		draw_info(0, COLOR_WHITE, op, "You need to mark raw materials to use.");
		return;
	}

	if (material->type != MATERIAL)
	{
		draw_info(0, COLOR_WHITE, op, "You can't use the marked item to build.");
		return;
	}
	...
	if (!new_arch)
	{
		...
		draw_info(0, COLOR_WHITE, op, "You can't use this strange material.");
		return;
	}
	...
	{
		draw_info(0, COLOR_WHITE, op, "You can't build there.");
		return;
	}
	...
	switch (material->sub_type)
	{
		...
		default:
			...
			draw_info(0, COLOR_WHITE, op, "Don't know how to apply this material, sorry.");
			break;
	}
...
static void construction_destroyer(object *op, int x, int y)
{
	...
	if (!item)
	{
		draw_info(0, COLOR_WHITE, op, "Nothing to remove.");
		return;
	}
	...
	{
		draw_info_format(0, COLOR_WHITE, op, "You cannot remove the %s, since it contains items.", query_name(item, NULL));
		return;
	}
	...
	draw_info_format(0, COLOR_WHITE, op, "You remove the %s.", query_name(item, NULL));
}
...
void construction_do(object *op, int dir)
{
	...
	if (!skill_item)
	{
		draw_info(0, COLOR_WHITE, op, "You need to apply a skill item to use this skill.");
		return;
	}

	if (skill_item->stats.sp != SK_CONSTRUCTION)
	{
		draw_info_format(0, COLOR_WHITE, op, "The %s cannot be used with the construction skill.", query_name(skill_item, NULL));
		return;
	}

	if (!dir)
	{
		draw_info(0, COLOR_WHITE, op, "You can't build or destroy under yourself.");
		return;
	}
	...
	{
		draw_info(0, COLOR_WHITE, op, "Can't build on map edge.");
		return;
	}
	...
	if (!floor)
	{
		...
		draw_info(0, COLOR_WHITE, op, "You'd better not build here, it looks weird.");
		return;
	}
	...
	if (skill_item->sub_type != ST_BD_REMOVE)
	{
		...
		if (!found_destroyer)
		{
			draw_info(0, COLOR_WHITE, op, "You cannot build without having a destroyer at hand.");
			return;
		}
	}
	...
	switch (skill_item->sub_type)
	{
		...
		default:
			...
			draw_info(0, COLOR_WHITE, op, "Don't know how to apply this tool, sorry.");
			break;
	}
}
