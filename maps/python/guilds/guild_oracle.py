...
def main():
	...
	# List the guild members.
		...
		if guild.guilddb[guild.guildname]["founder"]:
			activator.Write("Founder: {0}\n".format(guild.guilddb[guild.guildname]["founder"], COLOR_NAVY))
	...
	# Remove a member from the guild, or decline their membership application.
		...
		if not text[1]:
			...
			if member == None:
				me.SayTo(activator, "\nNo such member {0}.".format(member_name))
				...
				if guild.remove_member(member_name, me):
					me.SayTo(activator, "\nSuccessfully removed {0} from the guild.".format(member_name))
				else:
					me.SayTo(activator, "\nCould not remove {0} from the guild.".format(member_name))
		...
		if not text[1]:
			...
			if member == None:
				...
				if guild.approve_member(member_name):
					me.SayTo(activator, "\nSuccessfully approved {0} for full guild membership.".format(member_name))
				else:
					me.SayTo(activator, "\nSomething went wrong and {0} could not be approved.".format(member_name))
		...
		if not text[1]:
			...
			if member == None:
				...
				if guild.remove_admin(member_name, me):
					me.SayTo(activator, "\nSuccessfully removed administrator rights for {0}.".format(member_name))
				else:
					me.SayTo(activator, "\n{0} cannot have administrator rights taken away.".format(member_name))
		...
		if not text[1]:
			...
			if member == None:
				...
				if guild.make_admin(member_name):
					me.SayTo(activator, "\nSuccessfully made {0} the guild administrator.".format(member_name))
				else:
					me.SayTo(activator, "\n{0} cannot be made an administrator.".format(member_name))
		...
		if activator.f_wiz:
			if not text[1]:
				...
				if not PlayerExists(member_name):
					me.SayTo(activator, "\nThe player {0} does not exist.".format(member_name))
				elif member:
					me.SayTo(activator, "\nThe player {0} is already a member of the guild.".format(member_name))
				else:
					guild.add_member(member_name, guild.MEMBER_FLAG_NONE)
					me.SayTo(activator, "\nSuccessfully added {0} to the guild.".format(member_name))
		else:
			...
			if not text[1]:
				...
				if member == None:
					...
					if guild.make_founder(member_name):
						me.SayTo(activator, "\nSuccessfully made {0} the guild founder.".format(member_name))
					else:
						me.SayTo(activator, "\nCould not make {0} the guild founder.".format(member_name))
		else:
			me.SayTo(activator, "\nYou don't have enough privileges for this command.")
