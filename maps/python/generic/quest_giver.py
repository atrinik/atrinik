...
# Greeting
	...
	me.SayTo(activator, "\nHello %s, I am %s, the quest giver." % (activator.name, me.name))
	...
	if not qm.started():
		...
		if not quest_sack or not quest_sack.inventory:
			raise StandardError("Quest sack is missing or is empty for quest %s." % quest["info"]["quest_name"])
		...
		me.SayTo(activator, quest["messages"]["not_finished"] % (to_kill, to_kill > 1 and quest["messages"]["kill_suffix_pl"] or quest["messages"]["kill_suffix"]), 1)

# Accept the quest.
elif msg == "accept":
	if not qm.started():
		me.SayTo(activator, quest["messages"]["accepted"] % quest["info"]["kills"])
		qm.start()
