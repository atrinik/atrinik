...
def is_resource(s):
    for ext in (".png", ".s3m", ".ogg", ".mid", ".xm", ".mod"):
        if s.endswith(ext):
            if os.path.basename(os.path.dirname(s)) == "durations":
                return False
...
# Recursively scans directory for resources under the same license.
def dir_recurse(path, license, extra):
    for entry in os.listdir(path):
        entry = os.path.join(dirpath, to_remove, entry)

        # Resource, add it to the list.
        if is_resource(entry):
            licensed[license].append(entry)

            if extra:
                resource_info[entry] = extra
        # Go on recursively.
        elif os.path.isdir(entry):
            dir_recurse(entry, license, extra)

# Scan for resources and license files.
        ...
        for linenum, line in enumerate(fh, 1):
            ...
            if line.endswith(":\n"):
                ...
                if not license in licensed:
                    licensed[license] = []
            # Resource or directory name that is covered by the license.
                ...
                # There is more data about the resources(s).
                if len(parts) > 1:
                    extra = " ".join(parts[1:])
                ...
                # but scan the directory to add the resource names.
                ...
                # Simple resource name.
                    ...
                    if extra:
                        resource_info[to_remove_path] = extra
...
# Show list of unlicensed resources in verbose mode.
if verbose:
    print(colors.underscore + "List of unlicensed resources:" + colors.end)
    unlicensed.sort()
...
if not contributor:
    ...
    # Store length of licensed resources.
    licensed_len = 0
...
# Look for resources by specific contributor only.
else:
    license = None
    ...
    # Dump out the resources.
    for resource in licensed[license]:
        print("\t" + resource + (" " + resource_info[resource] if resource in resource_info else ""))
