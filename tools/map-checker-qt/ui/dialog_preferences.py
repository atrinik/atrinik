    ...
    def __init__(self, parent = None):
        ...
        for widget in self.findChildren(QToolButton):
            if widget.objectName().startswith("fileChooser_"):
                widget.clicked.connect(self.makeFileChooser(widget.objectName()[len("fileChooser_"):]))
    ...
    def settings_parse(self, save = False):
        for section in self.config.sections():
            for option in self.config.options(section):
                widget = self.findChild(QWidget, "pref_{0}_{1}".format(section.lower(), option))
                ...
                if type(widget) == QLineEdit:
                    ...
                    if save:
                        self.config.set(section, option, "yes" if widget.isChecked() else "no")
                    else:
    ...
    def makeFileChooser(self, name):
        def actionFileChooser():
            ...
            if option.startswith("path_dir_"):
                path = QFileDialog.getExistingDirectory(self, "Select Directory", widget.text(), QFileDialog.ShowDirsOnly | QFileDialog.DontResolveSymlinks)
            elif option.startswith("path_file_"):
                path = QFileDialog.getOpenFileName(self, "Select File", widget.text())[0]
