...
/** Flag mapping. */
static const flag_definition flags[] = {{"sleep", "FLAG_SLEEP"},
...
char *attack_save[NROFATTACKS] = {
    ...
    "electricity", "poison",   "acid",   "magic",  "mind",        "blind",     "paralyze",
...
static type_name type_names[] = {{"PLAYER", PLAYER},
                                 {"BULLET", BULLET},
                                 {"ROD", ROD},
                                 {"TREASURE", TREASURE},
                                 {"POTION", POTION},
...
static fake_name_definition fake_names[] = {{"animation", "animation_id"},
    ...
    while (fgets(buf, sizeof(buf), file)) {
        ...
        if (strstr(buf, "<description>") != NULL) {
            while (fgets(buf, sizeof(buf), file)) {
                ...
                if (type->description) {
                    type->description =
                        realloc(type->description, strlen(type->description) + strlen(buf) + 1);
        ...
        if (strstr(buf, "<attribute") != NULL) {
            ...
            while (fgets(buf, sizeof(buf), file)) {
                ...
                if (attr->description) {
                    attr->description =
                        realloc(attr->description, strlen(attr->description) + strlen(buf) + 1);
...
attribute_type *get_description_for_attribute(attribute_definition *attribute,
    ...
    for (desc = 0; desc < attribute->type_count; desc++) {
        ...
        if (description && attribute->types[desc]->description &&
            !strcmp(description, attribute->types[desc]->description)) {
            return attribute->types[desc];
        }
    }
...
static const char *in_living[] = {"exp",
...
static const char *custom_attributes[] = {"faction",
...
void write_type_file(type_definition *type) {
    ...
    for (attr = 0; attr < type->attribute_count; attr++) {
        ...
        fprintf(file,
                "</td>\n\t\t<td>%s\n\t\t</td>\n\t</tr>\n",
                type->attributes[attr]->description ? type->attributes[attr]->description
                                                    : "(no description)");
    }
...
void write_attribute_file(attribute_definition *attribute) {
    ...
    for (desc = 0; desc < attribute->type_count; desc++) {
        ...
        fprintf(file,
                "\t\t</td><td>%s</td>\n\t</tr>\n",
                attribute->types[desc]->description ? attribute->types[desc]->description
                                                    : "(no description)");
    }
...
int main(int argc, char **argv) {
    ...
    fallback_type->description =
        strdup("This type regroups all types who don't have a specific definition.");

    for (number = 0; number < type_count; number++) {
        for (attr = 0; attr < types[number]->attribute_count; attr++) {
            add_type_to_attribute(get_attribute(types[number]->attributes[attr]->field),
                                  types[number],
                                  attr);
        }
    }
