...
def _native_mutable_roots(root: Path, environment: Mapping[str, str],
                          ...
                          mounts: Mapping[str, MountInfo], failures: list[str],
    ...
    subjects = {path: True for path in paths}
    for path in extra:
        subjects.setdefault(path, False)
    if len(subjects) > 65536:
        raise ProbeError("native-mutable-roots-limit")
    for path, require_directory in subjects.items():
        ...
        while True:
            try:
                try:
                    ...
                    if require_directory or observed != path:
                        raise
                    descriptor = _native_open(observed, uid, directory=False)
                break
            except FileNotFoundError:
        ...
        try:
            ...
            if identity is None or (native and identity.filesystem not in NATIVE_FILESYSTEMS):
...
def probe(
    ...
    *,
    ...
    if mutable_roots:
        try:
            _native_mutable_roots(repository_root, environment, mutable_roots,
                                  uid, mounts, failures, native=False)
        except (OSError, ValueError, ProbeError):
            failures.append("mutable-root-proof-unavailable-or-unsafe")
...
def require_delivery_context(authority_root: Path, wrapper: Path, workspace: Path,
                             mutable_roots: tuple[Path, ...] = ()) -> dict[str, object]:
    """Fresh classification plus exact helper-selected mutable coordinates.

    The caller supplies paths derived from retained ownership records; this
    proves filesystem eligibility only, never ownership or reuse permission.
    """
    environment = dict(os.environ)
    environment["ATRINIK_WORKSPACE_DIR"] = str(workspace)
    roots = (wrapper, wrapper / "build", wrapper / "build/reviews", *mutable_roots)
    result = probe(authority_root, environment=environment, mutable_roots=roots)
    if result.get("authoritative") is not True or result.get("status") not in {
            CANONICAL_STATUS, NATIVE_LINUX_STATUS}:
        raise ProbeError("delivery-context-ineligible: " + ", ".join(result.get("failed_checks", [])))
    return result
