...
#define PATH_SP_MULT(op, spell)                         \
    ((((op)->path_attuned & (spell)->path) ? 0.8 : 1) * \
     (((op)->path_repelled & (spell)->path) ? 1.25 : 1))
/** Multiplier for spell damage based on the attenuation. */
#define PATH_DMG_MULT(op, spell)                         \
    ((((op)->path_attuned & (spell)->path) ? 1.25 : 1) * \
     (((op)->path_repelled & (spell)->path) ? 0.7 : 1))
